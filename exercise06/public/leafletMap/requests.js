 ...
 * @author Jakob Danel
 * @since 1.4.0
 * @update 1.6.0 Updated the RequestWeatherData class, to make the request not directly to the API, but to the server.
 */
...
class RequestWeatherData extends JQueryRequest {
     ...
     * 
     * 
     * UPDATE: The object did not request the data direct anymory, but make an request on the server.
     * @param {any[]} args Arguments will be used in the processsingFunction. Must be stored in an Array. By default an empty array is used.
     * @constructor
     */
    ...
    request() {
        let obj = this;
            $.post("/requestWeatherData",{
                lat : this.coordinates.latitude,
                lon: this.coordinates.longitude
            },(response) => {
                obj.processingFunction(response.response, obj.args);
            }); 
    }
