...
/**
 * constant that holds the left upper corner of the polygon as an Geojson object with the type Point.
 */
 const leftUpperCorner = makePointToGJSON(polygon[3]);

 /**
  * constant that holds the right bottom corner of the polygon as an Geojson object with the type Point.
  */
 const rightBottomCorner = makePointToGJSON(polygon[1]);
 
 /**
  * constant which holding the route data as geojson object with type LineString
  */
 const routeGJSON = makePointArrayToGJSON(route);
 
 /**
  * constant which holding the polygon data as geojson with type Polygon.
  */
 const polygonGJSON = makePolygonToGJSON(polygon);
/**
...
function isPointInsidePolygon(point, leftUpperCornerBBox = leftUpperCorner, rightBottomCornerBBox = rightBottomCorner) {
    return point[0] > leftUpperCornerBBox.coordinates[0] && point[0] < rightBottomCornerBBox.coordinates[0] && point[1] < leftUpperCornerBBox.coordinates[1] && point[1] > rightBottomCornerBBox.coordinates[1]
}
/**
 * 
...
function convertTableValues(points, leftUpperCornerBBox = leftUpperCorner, rightBottomCornerBBox = rightBottomCorner) {
    let rawData = getTableData(points.coordinates, leftUpperCornerBBox, rightBottomCornerBBox);
...
/**
 * Setting the content of the table object from the HTML page.
 * Converting the input geojson in internal format and calculate the subsequences.
 * @param {json} geojson data which should be showed in the table. 
 */
function showTable(geojson){
    document.getElementById("table").innerHTML = "<tbody></tbody>";
    
    document.getElementById("table").innerHTML = "<tr><th>Section</th><th>Length in Meters</th><th>First Position (Long,Lat)</th><th>Last Position (Long,Lat)</th><th>Is inside</th></tr><tbody id=\"tbody\"></tbody>";
    document.getElementById("tbody").innerHTML = makeTableHTML(convertTableValues(geojson));
    document.getElementById("totalLength").innerHTML ="Total length in meter: "+ round(calculateFullDistance(geojson.coordinates),2)+"m";
